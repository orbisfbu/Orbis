...
-(void)configureBaseViewsAndImage
{
    ...
    self.scrollViewOutlet.contentSize = CGSizeMake(self.view.frame.size.width, self.view.frame.size.height + 300);   
...
{
    self.registerButton = [[UIButton alloc] initWithFrame:CGRectMake(self.view.frame.size.width/2 - 75, self.eventImageView.frame.size.height + 10, 150, 40)];
...
-(void)createDistanceLabel
{
    ...
    [self.distanceFromUserLabel setText:@"Distance From You:"];
...
-(void)createdAttendanceCountLabel
{
    ...
    [self.attendanceCountLabel setText:@"Total Attendance:"];
...
-(void)createAgeRestrictionLabel
{
    ...
    [self.ageRestrictionLabel setText:@"Age Restriction:"];
...
{
    self.extraLabel = [[UILabel alloc] initWithFrame: CGRectMake(self.pollSectionLabel.frame.origin.x, self.view.frame.size.height, 40, 20)];
    //I say that the scroll in the scroll view should be disabled
    //until the user registers for event; then he or she can just
    //vote using certain buttons
    [self.pollSectionLabel setText:@"EXTRA STUFF"];
    [self.pollSectionLabel setFont:[UIFont fontWithName:@"GothamRounded-Bold" size:15]];
    [self.pollSectionLabel sizeToFit];
    [self.pollSectionLabel setRestorationIdentifier:@"extraLabel"];
...
{
    NSLog(@"Register button was pressed");
}
